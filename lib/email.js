...
export async function sendMarketplaceLeadNotificationToAdvisors(lead) {
  ...
  const rows = [
    ...
    ["איכות ליד", lead?.leadQuality],
    ["דירוג ליד", lead?.leadScoreTier],
    ["מחיר רכישה", formatCurrency(lead?.priceAtCreation)],
  ].filter(([, v]) => v && v !== "—");
  ...
  const body = `<p>ליד חדש זמין עכשיו בשוק הלידים של FINZO.</p>
    ${tableHtml}
    <p style="color:#64748b;font-size:12px;margin-top:16px;">פרטי יצירת קשר עם הלקוח יהיו זמינים רק לאחר רכישת הליד.</p>
    <div style="margin-top:24px;text-align:center;">
      <a href="${marketplaceUrl}" style="background:#7B3DFF;color:#fff;padding:12px 28px;border-radius:999px;text-decoration:none;font-weight:bold;display:inline-block;">כניסה לשוק הלידים</a>
    </div>`;

  const html = emailShell("ליד חדש בשוק הלידים", body);
