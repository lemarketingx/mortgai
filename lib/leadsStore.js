...
// Atomic purchase claim: updates store_status to 'sold' ONLY if the lead is
// currently available, sellable, and unclaimed. Returns the updated lead row,
// or null if no row matched (already sold / not sellable).
export async function claimLeadForPurchase(leadId, advisorId) {
  assertConfig();
  const now = new Date().toISOString();
  const patch = {
    store_status: "sold",
    buyer_advisor_id: advisorId,
    sold_at: now,
    last_updated: now,
  };
  // Supabase PATCH with all three guard conditions in the query string.
  // Returns [] (empty array) if no rows match — meaning the lead was already
  // claimed or is not sellable. Returns the updated row otherwise.
  // buyer_advisor_id is NOT NULL DEFAULT '' but guard against NULL too (pre-migration rows).
  // PostgREST OR syntax: or=(col.is.null,col.eq.value)
  const query = [
  ...
  if (!res.ok) {
    const body = await res.text().catch(() => "");
    logSupabase("claim_lead.error", { status: res.status, body });
    throw new LeadStoreError("SUPABASE_UPDATE_FAILED", `claimLeadForPurchase ${res.status}`, body);
  }

  const rows = await res.json().catch(() => []);
  if (!Array.isArray(rows) || rows.length === 0) return null; // already sold or conditions not met
