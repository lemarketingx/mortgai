...
// Known NUMERIC columns in the leads table (snake_case, as sent to Supabase).
...
// Removes "" from all known numeric columns so Supabase never receives an empty string
// for a numeric type. Converts "" → null (for nullable columns) or 0 if column was already 0.
function sanitizeRowForInsert(row) {
  const sanitized = { ...row };
  const emptyNumericFields = [];
  for (const col of NUMERIC_COLUMNS) {
    if (Object.prototype.hasOwnProperty.call(sanitized, col) && sanitized[col] === "") {
      emptyNumericFields.push(col);
      sanitized[col] = null;
    }
    // Also guard against undefined leaking through
    if (Object.prototype.hasOwnProperty.call(sanitized, col) && sanitized[col] === undefined) {
      sanitized[col] = null;
    }
  }
  if (emptyNumericFields.length > 0) {
    logSupabase("insert.sanitize.empty_numeric_fields", { emptyNumericFields, payloadColumns: Object.keys(sanitized) });
  }
  return sanitized;
}
...
export async function createLead(payload = {}) {
  ...
  const candidate = sanitizeRowForInsert(toRow(record));
