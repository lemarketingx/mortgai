...
function buildScenarioSummary(lead = {}, analysis = {}) {
  const scenario = lead.mortgageScenario || analysis.scenarioLabel || "";
  if (!scenario) return "";
  const maxLtv = lead.scenarioMaxLtv ?? analysis.scenarioMaxLtv;
  const actualLtv = toOptionalNumber(lead.actualLtv ?? analysis.ltv);
  const equityGap = toOptionalNumber(lead.requiredEquityGap ?? analysis.missingEquity);

  const parts = [`סוג עסקה: ${scenario}`];
  parts.push(`שיעור מימון מקסימלי: ${maxLtv ? `${Math.round(maxLtv)}%` : "נדרש בדיקה פרטנית מול יועץ"}`);
  if (actualLtv) parts.push(`שיעור מימון בפועל: ${Math.round(actualLtv)}%`);
  if (equityGap > 0) parts.push(`פער הון עצמי נדרש: ${Math.round(equityGap)} ₪`);
  return parts.join(" | ");
}
...
export async function createLead(payload = {}) {
  ...
  record.previewSummary = lead.previewSummary || buildScenarioSummary(lead, analysis);
