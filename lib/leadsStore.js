...
export async function readStoreLeads() {
  ...
  return available.map((lead) => {
    // Compute FINZO score and price. Pass boolean flags for name/phone
    // so the scoring engine measures completeness without exposing PII.
    ...
    // If admin has manually set store_price > 0, honour it; otherwise use computed price.
    const finalPrice = lead.storePrice > 0 ? lead.storePrice : pricing.price;
...
export async function createLeadPurchase({ leadId, advisorId, purchaseType = "regular", price = 0, isExclusive = false }) {
  ...
  if (!res.ok) {
    ...
    // Unique constraint violation — another advisor already purchased this lead
    if (res.status === 409 || body.includes("23505") || body.includes("duplicate key")) {
      throw new LeadStoreError("LEAD_ALREADY_SOLD", "הליד כבר נרכש על ידי יועץ אחר.", body);
    }
