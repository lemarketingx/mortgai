...
// Minimal column set for the client upload portal — only what the portal APIs need.
const LEADS_CLIENT_COLUMNS = [
  "id", "name", "mortgage_type", "employment_status", "pipeline_stage", "assigned_advisor",
];

// Client-portal variant: fetches only the 6 columns needed by /api/client/case/[token]/info
// and /api/client/documents/upload. Reduces payload by ~92% vs full readMyLeads.
export async function readMyLeadsForClient(advisorId) {
  assertConfig();
  const url = endpoint(LEAD_PURCHASES_TABLE, `advisor_id=eq.${encodeURIComponent(advisorId)}&order=purchased_at.desc`);
  let res;
  try {
    ...
    logFetchError("select.my_leads_client", error);
    throw new LeadStoreError("SUPABASE_READ_FAILED", "readMyLeadsForClient fetch failed", error?.message || String(error));
  }

  if (!res.ok) {
    const body = await res.text().catch(() => "");
    if (isTableMissingError(body)) return [];
    throw new LeadStoreError("SUPABASE_READ_FAILED", `readMyLeadsForClient ${res.status}`, body);
  }

  const payload = await safeReadResponse("select.my_leads_client.success", res);
  const purchases = Array.isArray(payload.parsed) ? payload.parsed : [];
  if (purchases.length === 0) return [];

  const purchasedLeadIds = new Set(purchases.map((p) => p.lead_id).filter(Boolean));
  const allLeads = await readLeadsWithColumns(LEADS_CLIENT_COLUMNS);

  return allLeads.filter((lead) => purchasedLeadIds.has(lead.id));
}
