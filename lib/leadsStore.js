...
const LEADS_SELECT_COLUMNS = [
  ...
  // FINZO Lead Score v2
  "email", "monthly_obligations", "desired_monthly_payment", "process_stage", "preferred_contact_method",
  "consent_advisor_contact", "lead_score", "lead_score_tier", "lead_score_breakdown",
  "price_at_creation", "score_version", "is_sellable", "quality_notes",
];
...
const NUMERIC_NULLABLE_COLS = new Set([
  ...
  // FINZO Lead Score v2
  "monthly_obligations", "desired_monthly_payment", "lead_score", "price_at_creation",
]);
...
// ─── Shared timestamp sanitizer ───────────────────────────────────────────────
// Used by both insert and update paths. Guarantees no TIMESTAMPTZ/DATE column
// receives "" / undefined / an invalid date string — Postgres/PostgREST rejects
// those with a 400 instead of treating them as null.
function sanitizeTimestampColumns(row, diagnostics = null) {
  ...
  for (const col of Object.keys(row)) {
    ...
    if (TIMESTAMP_OPTIONAL_COLS.has(col)) {
      if (value === "" || value === undefined) {
        diagnostics?.[value === "" ? "emptyTimestampFieldsFixed" : "undefinedFieldsFixed"].push(col);
        row[col] = null;
...
function sanitizeLeadRowForInsert(row) {
  ...
  const sanitized = sanitizeTimestampColumns({ ...row }, diagnostics);

  for (const col of Object.keys(sanitized)) {
    if (TIMESTAMP_REQUIRED_COLS.has(col) || TIMESTAMP_OPTIONAL_COLS.has(col)) continue;
...
function toRow(record) {
  return {
    ...
    // FINZO Lead Score v2
    email: record.email,
    monthly_obligations: record.monthlyObligations,
    desired_monthly_payment: record.desiredMonthlyPayment,
    process_stage: record.processStage,
    preferred_contact_method: record.preferredContactMethod,
    consent_advisor_contact: Boolean(record.consentAdvisorContact),
    lead_score: record.leadScore,
    lead_score_tier: record.leadScoreTier,
    lead_score_breakdown: record.leadScoreBreakdown,
    price_at_creation: record.priceAtCreation,
    score_version: record.scoreVersion,
    is_sellable: Boolean(record.isSellable),
...
function fromRow(row = {}) {
  ...
  const lead = {
    ...
    // FINZO Lead Score v2
    email: row.email || "",
    monthlyObligations: toOptionalNumber(row.monthly_obligations),
    desiredMonthlyPayment: toOptionalNumber(row.desired_monthly_payment),
    processStage: row.process_stage || "",
    preferredContactMethod: row.preferred_contact_method || "",
    consentAdvisorContact: toBool(row.consent_advisor_contact),
    leadScore: toOptionalNumber(row.lead_score),
    leadScoreTier: row.lead_score_tier || "",
    leadScoreBreakdown: row.lead_score_breakdown || null,
    priceAtCreation: toOptionalNumber(row.price_at_creation),
...
// Core leads select with the missing-column retry loop. `extraFilter` is an
// optional PostgREST filter fragment (e.g. "id=in.(...)" or "store_status=eq.available")
// so callers can push filtering to the database instead of reading the full table.
async function fetchLeads(extraFilter = "") {
  assertConfig();
  const columns = [...LEADS_SELECT_COLUMNS];
  const filterPart = extraFilter ? `&${extraFilter}` : "";

  while (columns.length > 0) {
    const query = `select=${encodeURIComponent(columns.join(","))}${filterPart}&order=created_at.desc`;
...
export async function readLeadById(leadId) {
  const [lead] = await fetchLeads(`id=eq.${encodeURIComponent(leadId)}`);
  return lead || null;
}

export async function readLeadsByAssignedAdvisor(advisorId) {
  return fetchLeads(`assigned_advisor_id=eq.${encodeURIComponent(advisorId)}`);
}
...
export async function createLead(payload = {}) {
  ...
  const record = {
    ...
    // FINZO Lead Score v2 — set by pages/api/lead.js before calling createLead
    email: lead.email || "",
    monthlyObligations: toOptionalNumber(lead.monthlyObligations ?? lead.monthly_obligations),
    desiredMonthlyPayment: toOptionalNumber(lead.desiredMonthlyPayment ?? lead.desired_monthly_payment),
    processStage: lead.processStage || lead.process_stage || "",
    preferredContactMethod: lead.preferredContactMethod || lead.preferred_contact_method || "",
    consentAdvisorContact: toBool(lead.consentAdvisorContact ?? lead.consent_advisor_contact),
    leadScore: lead.leadScore != null ? Number(lead.leadScore) : null,
    leadScoreTier: lead.leadScoreTier || "",
    leadScoreBreakdown: lead.leadScoreBreakdown || null,
    priceAtCreation: lead.priceAtCreation != null ? Number(lead.priceAtCreation) : null,
    scoreVersion: lead.scoreVersion || "",
    isSellable: toBool(lead.isSellable),
  ...
  const emptyMsg = `All ${removedColumns.length} columns dropped — migration likely missing. Removed: ${removedColumns.join(", ")}`;
  console.error("[supabase:insert.leads.empty_payload]", { removedColumns });
  throw new LeadStoreError("SUPABASE_INSERT_EMPTY_PAYLOAD", emptyMsg, JSON.stringify({ removedColumns }));
}

export async function updateLead(id, changes = {}) {
  assertConfig();
  const patch = sanitizeTimestampColumns(toPartialRow(changes));
...
// Atomic exclusive lock: updates store_status only if the lead is still
// available when the database UPDATE runs.
export async function lockLeadForPurchase(leadId, { buyerAdvisorId, storeStatus = "sold", requireSellable = false }) {
  assertConfig();
  const now = new Date().toISOString();
  const patch = {
    store_status: storeStatus,
    buyer_advisor_id: buyerAdvisorId,
    sold_at: now,
    last_updated: now,
  };
  // Returns [] if no row matched, which means another request already won.
  const query = [
    `id=eq.${encodeURIComponent(leadId)}`,
    `store_status=eq.available`,
    `or=(buyer_advisor_id.is.null,buyer_advisor_id.eq.)`,
    ...(requireSellable ? [`is_sellable=eq.true`] : []),
  ].join("&");

  const res = await fetch(endpoint(LEADS_TABLE, query), {
    method: "PATCH",
    headers: { ...baseHeaders(), Prefer: "return=representation" },
    body: JSON.stringify(patch),
  });
  ...
  if (!res.ok) {
    const body = await res.text().catch(() => "");
    logSupabase("lock_lead_for_purchase.error", { status: res.status, body });
    throw new LeadStoreError("SUPABASE_UPDATE_FAILED", `lockLeadForPurchase ${res.status}`, body);
  }

  const rows = await res.json().catch(() => []);
  return Array.isArray(rows) && rows.length === 1 ? fromRow(rows[0]) : null;
}
...
export async function readStoreLeads() {
  // store_status filter runs in the database; a null store_status maps to
  // "pending_review" in fromRow, so eq.available matches the same rows the
  // old in-memory filter did. isSellable stays in memory (derived field).
  const availableLeads = await fetchLeads("store_status=eq.available");
  // Only show leads that are sellable (score >= 41) and available for purchase
  const available = availableLeads.filter((lead) => lead.isSellable && lead.storeStatus === "available");

  return available.map((lead) => ({
    id:               lead.id,
    createdAt:        lead.createdAt,
    city:             lead.city,
    // Derived, PII-free preview number — raw income/payment are never exposed
    repaymentRatio:   Number(lead.monthlyIncome) > 0 && Number(lead.estimatedPayment) > 0
                        ? Math.round((Number(lead.estimatedPayment) / Number(lead.monthlyIncome)) * 100)
                        : null,
    mortgageAmount:   lead.mortgageAmount,
    propertyPrice:    lead.propertyPrice,
    equityAmount:     lead.equityAmount != null && lead.equityAmount !== ""
                        ? Number(lead.equityAmount)
                        : Math.max(0, Number(lead.propertyPrice || 0) - Number(lead.mortgageAmount || 0)) || null,
    ...
    storePrice:       lead.priceAtCreation || 0,
    exclusivePrice:   lead.priceAtCreation || 0,
    contractStatus:   lead.contractStatus || "",
    employmentStatus: lead.employmentStatus || "",
    purchaseStatus:   lead.purchaseStatus || "",
    processStage:     lead.processStage || "",
    // Score fields for advisor UI
    leadScore:        lead.leadScore,
    leadScoreTier:    lead.leadScoreTier || "",
    qualityNotes:     lead.qualityNotes || "",
    priceAtCreation:  lead.priceAtCreation,
    // Legacy compat fields (used by advisor/leads.js)
    finzoScore:       lead.leadScore,
    pricingBullets:   lead.qualityNotes ? [lead.qualityNotes] : [],
  }));
}
...
export async function createLeadPurchase({ leadId, advisorId, purchaseType = "regular", price = 0, isExclusive = false }) {
  ...
  // The caller must lock the lead before inserting the purchase row.
  // Keeping this function insert-only avoids a JS check-then-update race.
...
export async function readMyLeads(advisorId) {
  ...
  // First purchase per lead wins (list is ordered purchased_at.desc, matching
  // the previous purchases.find behavior which took the first match).
  const purchaseByLeadId = new Map();
  for (const p of purchases) {
    if (p.lead_id && !purchaseByLeadId.has(p.lead_id)) purchaseByLeadId.set(p.lead_id, p);
  }

  // Fetch only the purchased leads instead of the full table. Chunked so the
  // id=in.(...) filter keeps request URLs bounded for advisors with many leads.
  const ids = [...purchaseByLeadId.keys()];
  const CHUNK_SIZE = 100;
  const chunks = [];
  for (let i = 0; i < ids.length; i += CHUNK_SIZE) {
    const chunk = ids.slice(i, i + CHUNK_SIZE);
    chunks.push(fetchLeads(`id=in.(${chunk.map(encodeURIComponent).join(",")})`));
  }
  const leads = (await Promise.all(chunks)).flat();
  // Each chunk arrives sorted; re-sort merged result to keep created_at.desc order
  leads.sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0));

  return leads
    .filter((lead) => purchaseByLeadId.has(lead.id))
    .map((lead) => mergePurchaseFields(lead, purchaseByLeadId.get(lead.id)));
}

function mergePurchaseFields(lead, purchase) {
  return {
    ...lead,
...
// Single-lead version of readMyLeads: verifies the advisor purchased `leadId`
// and returns that lead (with purchase fields) or null. Two tiny indexed
// queries instead of reading the advisor's whole lead list.
export async function readMyLead(advisorId, leadId) {
  assertConfig();
  const url = endpoint(
    LEAD_PURCHASES_TABLE,
    `advisor_id=eq.${encodeURIComponent(advisorId)}&lead_id=eq.${encodeURIComponent(leadId)}&order=purchased_at.desc&limit=1`
  );

  // The ownership lookup (lead_purchases) and the lead fetch are independent
  // queries, so run them concurrently. allSettled (not all) preserves the exact
  // original semantics: if there is no purchase we return null and ignore the
  // lead result entirely — the lead is never returned to an advisor who didn't
  // buy it, and a lead-fetch failure on a non-owned lead does not change the
  // response. The lead query only matters after the ownership gate passes.
  const purchaseFetch = (async () => {
    try {
      return await fetch(url, { method: "GET", headers: baseHeaders() });
    } catch (error) {
      logFetchError("select.my_lead", error);
      throw new LeadStoreError("SUPABASE_READ_FAILED", "readMyLead fetch failed", error?.message || String(error));
    }
  })();
  const [purchaseSettled, leadSettled] = await Promise.allSettled([
    purchaseFetch,
    fetchLeads(`id=eq.${encodeURIComponent(leadId)}`),
  ]);

  // Ownership-lookup errors propagate exactly as in the sequential version.
  if (purchaseSettled.status === "rejected") throw purchaseSettled.reason;
  const res = purchaseSettled.value;
  if (!res.ok) {
    const body = await res.text().catch(() => "");
    if (isTableMissingError(body)) return null;
    throw new LeadStoreError("SUPABASE_READ_FAILED", `readMyLead ${res.status}`, body);
  }

  const payload = await safeReadResponse("select.my_lead.success", res);
  const purchase = Array.isArray(payload.parsed) ? payload.parsed[0] : null;
  if (!purchase) return null; // ownership gate — no purchase, lead never returned

  // Only now does the lead result matter; surface its error just like before.
  if (leadSettled.status === "rejected") throw leadSettled.reason;
  const lead = leadSettled.value?.[0];
  if (!lead) return null;
  return mergePurchaseFields(lead, purchase);
}
