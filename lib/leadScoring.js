...
function scoreCompleteness(d) {
  ...
  if (Number(d.propertyValue || d.propertyPrice) > 0)       pts += 3; // שווי נכס
...
function scoreFinancial(d) {
  ...
  const equity         = Number(d.equityAmount) > 0
    ? Number(d.equityAmount)
    : propertyValue > 0 && mortgageAmount > 0 ? Math.max(0, propertyValue - mortgageAmount) : 0;
