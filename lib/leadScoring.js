...
const SCORE_BANDS = [
  { min: 91, quality: "פרימיום",  price: 599 },
  { min: 76, quality: "חם מאוד", price: 399 },
  { min: 61, quality: "חם",       price: 249 },
  { min: 41, quality: "בינוני",   price: 149 },
  { min: 0,  quality: "הזדמנות", price: 99  },
];
...
// ─── Full pricing computation (score + value adjustments + breakdown) ──────────
export function computePricing(lead) {
  const { total: score, completeness, financial, typeScore } = computeFinzoScore(lead);
  const band = getBand(score);

  // Small upward adjustments for high-value signals (never discount)
  let adj = 0;
  const mortgage = Number(lead.mortgageAmount) || 0;
  if (mortgage >= 2000000) adj += 0.15;
  else if (mortgage >= 1000000) adj += 0.08;

  const propPrice = Number(lead.propertyPrice) || 0;
  if (propPrice >= 3000000) adj += 0.12;
  else if (propPrice >= 2000000) adj += 0.06;

  const days = Math.floor((Date.now() - new Date(lead.createdAt || 0).getTime()) / 86400000);
  if (days <= 3) adj += 0.08;

  if (completeness >= 24) adj += 0.08;

  // Round to nearest 10₪
  const price = Math.round(band.price * (1 + adj) / 10) * 10;
