...
const ADVISOR_REVIEW_NOTE = "נדרש בדיקה פרטנית מול יועץ";

export const MORTGAGE_SCENARIOS = {
  first_home: { label: "רכישת דירה ראשונה / יחידה", maxLtv: 75, requiresAdvisorReview: false, notes: "" },
  upgrade: { label: "שיפור דיור", maxLtv: 70, requiresAdvisorReview: false, notes: "" },
  investment: {
    label: "דירה להשקעה",
    maxLtv: 50,
    requiresAdvisorReview: false,
    notes: "ברכישת דירה להשקעה שיעור המימון המקסימלי בדרך כלל נמוך יותר ועומד עד 50% משווי הנכס.",
  },
  refinance: { label: "מחזור משכנתא", maxLtv: null, requiresAdvisorReview: true, notes: ADVISOR_REVIEW_NOTE },
  any_purpose: { label: "לכל מטרה", maxLtv: null, requiresAdvisorReview: true, notes: ADVISOR_REVIEW_NOTE },
};

export function getMortgageRulesByScenario(scenario) {
  const rules = MORTGAGE_SCENARIOS[scenario] || MORTGAGE_SCENARIOS.first_home;
  return {
    maxLtv: rules.maxLtv,
    label: rules.label,
    requiresAdvisorReview: !!rules.requiresAdvisorReview,
    notes: rules.notes || "",
  };
}
...
export function calculateMortgageAnalysis(data, rates = {}) {
  ...
  const scenario = MORTGAGE_SCENARIOS[data.propertyType] ? data.propertyType : "first_home";
  const scenarioRules = getMortgageRulesByScenario(scenario);
  const isRefinance = scenario === "refinance";
  const existingPropertyValue = toPositiveNumber(data.existingPropertyValue);
  const existingMortgageBalance = toPositiveNumber(data.existingMortgageBalance);
  ...
  const ltvBasisPrice = isRefinance && existingPropertyValue ? existingPropertyValue : price;
  const ltvBasisMortgage = isRefinance && existingMortgageBalance ? existingMortgageBalance : mortgage;

  const maxMortgage = scenarioRules.maxLtv && ltvBasisPrice ? Math.round(ltvBasisPrice * (scenarioRules.maxLtv / 100)) : 0;
  const requestedAboveLimit = scenarioRules.maxLtv && ltvBasisPrice ? Math.max(ltvBasisMortgage - maxMortgage, 0) : 0;
  const missingEquity = Math.max(requestedAboveLimit, 0);
  ...
  const ltv = ltvBasisPrice ? (ltvBasisMortgage / ltvBasisPrice) * 100 : 0;
  const ltvLimitExceeded = scenarioRules.maxLtv ? ltv > scenarioRules.maxLtv : false;
  ...
  const mainIssue = scenarioRules.requiresAdvisorReview
    ? scenarioRules.notes
    : hardLimitExceeded
  ...
  const recommendedAction = scenarioRules.requiresAdvisorReview
    ? scenarioRules.notes
    : hardLimitExceeded || borderlineCase
  ...
  const analysis = {
    ...
    scenario,
    scenarioLabel: scenarioRules.label,
    scenarioMaxLtv: scenarioRules.maxLtv,
    scenarioRequiresAdvisorReview: scenarioRules.requiresAdvisorReview,
    scenarioNotes: scenarioRules.notes,
