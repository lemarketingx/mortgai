...
function buildRecommendations(analysis) {
  ...
  if (analysis.loansToCloseMonthly > 0) {
    recommendations.push("סגירת הלוואות עשויה לשפר את יחס ההתחייבויות ואת אומדן האישור.");
  }

  if (!recommendations.length) {
    recommendations.push("לפי הנתונים שהוזנו התיק נראה מאוזן. כדאי להשוות כמה הצעות בנקאיות ולבדוק תמהיל לפני חתימה.");
  }
...
export function calculateMortgageAnalysis(data, rates = {}) {
  ...
  const risk = hardLimitExceeded ? "סיכון גבוה" : borderlineCase ? "תיק גבולי - עשוי לדרוש התאמות" : approval >= 75 ? "בסיס טוב לבדיקה" : "דורש בדיקה";
  ...
  const recommendedAction = hardLimitExceeded || borderlineCase
    ? "תיק גבולי - עשוי לדרוש התאמות. מומלץ להקטין משכנתא, להאריך תקופה, להפחית הלוואות או להגדיל הכנסה לפני פנייה לבנק."
    : missingEquity > 0
      ? "להגדיל הון עצמי או לבחור נכס זול יותר."
      : approval < 60
        ? "לשפר נתונים לפני פנייה לבנק."
        : "התיק נראה מתאים לבדיקה מקצועית מול כמה בנקים.";
