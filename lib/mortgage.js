...
export function calculateEarlyPaymentPenalty(remainingPrincipal, annualRate, remainingMonths, penaltyType = "standard") {
  ...
  const penalty = Math.round(principal * monthlyRate * differentialMonths * 0.6);

  return { penalty, differentialMonths, description: `עמלת היוון: ${differentialMonths} חודשים × 60% מהפרש הריביות` };
}
...
export function refinanceRecommendation(currentLoan, newTerms) {
  ...
  const penalty = calculateEarlyPaymentPenalty(principal, rate, remainingYears * 12, trackType);
