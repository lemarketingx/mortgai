...
export async function updateDocument(id, {
  status, notes,
  // File upload fields (client portal)
  storagePath, fileName, fileSize, mimeType, uploadedBy, uploadedAt,
  // Advisor review fields
  ...
  if (fileSize !== undefined) patch.file_size = fileSize;
  if (mimeType !== undefined) patch.mime_type = mimeType;
  if (uploadedBy !== undefined) patch.uploaded_by = uploadedBy;
  if (uploadedAt !== undefined) patch.uploaded_at = uploadedAt;
