...
// redirectTo: optional URL Supabase appends the recovery token to after verification.
export async function supabasePasswordReset(email, redirectTo) {
  assertConfig();
  try {
    const body = { email };
...
// Update the password for an authenticated user using their recovery access token.
// accessToken: the token from the recovery email redirect fragment (#access_token=…)
export async function supabaseUpdatePassword(accessToken, newPassword) {
  ...
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
    return { error: data.error_description || data.msg || data.message || "Update failed" };
  }
  return { error: null };
}
