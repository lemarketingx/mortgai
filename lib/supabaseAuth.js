...
//
// redirect_to points the recovery email link back to /advisor/reset-password,
// which reads the access_token from the URL and lets the advisor set a new password.
...
// Update the password for the user identified by a Supabase recovery access_token.
// Called from /api/advisor/reset-password after the advisor follows the email link.
export async function supabaseUpdatePasswordWithToken(accessToken, newPassword) {
  assertConfig();
  ...
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
    const msg = data.error_description || data.msg || data.message || "Password update failed";
