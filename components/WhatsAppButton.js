// Floating WhatsApp support button for the public (consumer) site.
// Renders only when NEXT_PUBLIC_WHATSAPP_NUMBER is set, so it stays hidden
// until a support number is configured. Number format: international digits
// only, e.g. "972501234567" (no +, spaces or dashes).

const RAW_NUMBER = process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || "";
const NUMBER = RAW_NUMBER.replace(/[^\d]/g, "");
const PREFILL = process.env.NEXT_PUBLIC_WHATSAPP_MESSAGE || "היי, אשמח לעזרה בנוגע ל-FINZO";
...
export default function WhatsAppButton() {
  ...
  return (
    <a
      ...
      aria-label="פנייה לתמיכה בוואטסאפ"
      className="fixed bottom-5 left-5 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] shadow-lg shadow-[#25D366]/30 transition hover:scale-105 hover:bg-[#1ebe5b] focus:outline-none focus-visible:ring-4 focus-visible:ring-[#25D366]/40"
    >
