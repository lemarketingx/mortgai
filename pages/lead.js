...
const initialLead = {
  name: "", phone: "", email: "",
  purchaseStatus: "", processStage: "",
  propertyPrice: "", equityAmount: "", mortgageAmount: "",
...
export default function LeadPage() {
  ...
  useEffect(() => {
    try {
      // Load calculator prefill first, then override with any saved draft
  ...
  async function submit(event) {
    ...
    if (!lead.consentAdvisorContact) { setError("יש לאשר הסכמה לחזרה של יועץ לפני שליחה."); return; }
    setLoading(true);
    setError("");

    const mortgageAmount = lead.mortgageAmount
      ? Number(String(lead.mortgageAmount).replace(/[^\d]/g, "")) || 0
      : Math.max(0, (Number(String(lead.propertyPrice || "0").replace(/[^\d]/g, "")) || 0) - (Number(String(lead.equityAmount || "0").replace(/[^\d]/g, "")) || 0));
    ...
    const payload = {
      ...
      propertyPrice: Number(String(lead.propertyPrice || "0").replace(/[^\d]/g, "")) || 0,
      equityAmount: Number(String(lead.equityAmount || "0").replace(/[^\d]/g, "")) || 0,
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_300px]">
          <form onSubmit={submit} aria-busy={loading ? "true" : "false"}
            ...
            <h1 className="mt-4 text-3xl font-black tracking-tight sm:text-4xl">
              {step === 1 ? "פרטים בסיסיים" : step === 2 ? "נתונים פיננסיים" : "שלב התהליך ויצירת קשר"}
            </h1>
            <p className="mt-2 max-w-2xl text-sm font-bold leading-7 text-slate-600 dark:text-slate-400">
              {step === 1 ? "נתחיל עם הפרטים הבסיסיים שלך" : step === 2 ? "עכשיו נבדוק את הנתונים הפיננסיים" : "ספר לנו היכן אתה בתהליך ואיך הכי נוח לחזור אליך"}
            </p>
            ...
            {step === 1 && (
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                ...
                <TextField label="עיר" value={lead.city} onChange={(v) => update("city", v)} placeholder="תל אביב" className="sm:col-span-2" />
              </div>
            )}
            ...
            {step === 2 && (
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                ...
                <MoneyField label="החזרי הלוואות חודשיים" value={lead.monthlyObligations} onChange={(v) => update("monthlyObligations", v)} placeholder="0 אם אין" />
                <MoneyField label="סכום משכנתא מבוקש" value={lead.mortgageAmount} onChange={(v) => update("mortgageAmount", v)} placeholder="יחושב אוטומטית" />
            ...
            {/* Step 3: Stage + Contact */}
            {step === 3 && (
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                <SelectField label="באיזה שלב אתה בתהליך?" value={lead.processStage} onChange={(v) => update("processStage", v)} options={PROCESS_STAGE_OPTIONS} className="sm:col-span-2" />
                <SelectField label="מועד נוח לחזרה" value={lead.requestedContactTime} onChange={(v) => update("requestedContactTime", v)} options={CONTACT_TIME_OPTIONS} />
                <SelectField label="דרך יצירת קשר מועדפת" value={lead.preferredContactMethod} onChange={(v) => update("preferredContactMethod", v)} options={CONTACT_METHOD_OPTIONS} />
                <TextField label='אימייל (אופציונלי)' value={lead.email} onChange={(v) => update("email", v)} placeholder="you@example.com" type="email" className="sm:col-span-2" />
                ...
                <label className="mt-2 flex items-start gap-3 cursor-pointer sm:col-span-2">
                  <input type="checkbox" checked={lead.consentAdvisorContact} onChange={(e) => update("consentAdvisorContact", e.target.checked)} className="mt-0.5 w-4 h-4 accent-violet-600 shrink-0" />
